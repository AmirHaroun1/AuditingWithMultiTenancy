...
<div>
    <div class="col-md-3">
        <div>
            ...
            <div>
                <virtual-list style="height: 500px; overflow-y: auto;" v-on="$listeners" :data-key="'id'" :data-sources="ListItems" :data-component="ItemComponent" />
            </div>
        </div>
    </div>
    <div class="col-md-9" v-if="SelectedItem">
        <div>
            ...
            <v-card dir="rtl" shaped id="printHeading" :elevation="24">
                <v-container>
                    ...
                    <div class="text-center">
                        ...
                        <v-card-subtitle>
                            {{$t('grade')}}: <b class="primary--text">{{TotalMarks}}</b>/{{SelectedItem.mark}}
                        </v-card-subtitle>

                        <br>
                        <v-card-text>
                            {{SelectedItem.first_note}}
                        </v-card-text>
                        <hr>
                        <v-card-text>
                            {{SelectedItem.second_note}}
                        </v-card-text>
                    </div>
                    <div class="box-body">
                        <!------------ in case of files -------------------->
                                            ...
                                            <tr v-for="(ChildItem,index) in item.children">
                                                ...
                                                <td v-if="ChildItem.isText == 1" colspan="3">
                                                    ...
                                                    <div v-if="!PrintingMood">
                                                        <textarea outlined dense v-if="!ChildItem.transactions.length" v-model="item.text" class="form-control"></textarea>

                                                        <textarea outlined dense v-else v-model="ChildItem.transactions[0].pivot.text" class="form-control"></textarea>

                                                    </div>
                                                    <div v-else>
                                                        <textarea outlined dense v-if="!ChildItem.transactions.length" class="form-control">{{item.text}}</textarea>

                                                        <textarea outlined dense v-else class="form-control">{{ChildItem.transactions[0].pivot.text}}</textarea>
                                                    ...
                                                    <div class="mt-10" v-if="ChildItem.children.length">
                                                        <div v-for="innerItem in ChildItem.children">
                                                            <hr>
                                                            <div v-if="!PrintingMood">
                                                                <textarea outlined dense v-if="!innerItem.transactions.length" v-model="innerItem.text" class="form-control mb-6"></textarea>

                                                                <textarea outlined dense v-else v-model="innerItem.transactions[0].pivot.text" class="form-control mb-6"></textarea>
                                                                <v-btn @click="innerItem.transactions.length? UpdateExistingTransactionArchiveRecord(innerItem,0) : AddNewTransactionArchiveRecord(innerItem,0)" class="ma-4" color="primary" dark>
                                                                    {{$t('save')}}
                                                                </v-btn>
                                                            </div>
                                                            <div v-else>
                                                                <textarea outlined dense v-if="!innerItem.transactions.length" class="form-control">{{innerItem.text}}</textarea>

                                                                <textarea outlined dense v-else class="form-control">{{innerItem.transactions[0].pivot.text}}</textarea>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </td>
                                                ...
                                                <td v-if="ChildItem.isText == 1 && !PrintingMood">
                                                    <v-btn @click="ChildItem.transactions.length? UpdateExistingTransactionArchiveRecord(ChildItem,null) : AddNewTransactionArchiveRecord(ChildItem,null)" class="ma-4" color="primary" dark>
                                                        {{$t('save')}}
                                                    </v-btn>
                                                    <a class="btn btn-success btn-sm">حفظ</a>
                                                </td>
                                                <!--------- ./if it's a text area --------->
                                    ...
                                    <tbody>
                                        <tr v-for="item in SelectedItem.children">
                                            ...
                                            <td colspan="3" v-if="item.isText == 1">
                                                <div v-if="!PrintingMood">
                                                    <textarea outlined dense v-if="!item.transactions.length" v-model="item.text" class="form-control mb-6"></textarea>

                                                    <textarea outlined dense v-else v-model="item.transactions[0].pivot.text" class="form-control mb-6"></textarea>

                                                </div>
                                                <div v-else>
                                                    <textarea outlined dense v-if="!item.transactions.length" class="form-control mb-6">{{item.text}}</textarea>

                                                    <textarea outlined dense v-else class="form-control mb-6">{{item.transactions[0].pivot.text}}</textarea>
                                            ...
                                            <td v-if="item.isText == 1 && !PrintingMood">
                                                <v-btn @click="item.transactions.length? UpdateExistingTransactionArchiveRecord(item,null) : AddNewTransactionArchiveRecord(item,null)" color="success" dark>
                                                    {{$t('save')}}
                                                </v-btn>
                                            </td>
                                            <!--------- ./if it's a text area --------->
                                                ...
                                                <div v-if="item.children.length">
                                                    ...
                                                    <div class="mt-10" v-for="innerItem in item.children">
                                                        ...
                                                        <div v-else-if="innerItem.isText == 1">
                                                            <div v-if="!PrintingMood">
                                                                <textarea outlined dense v-if="!innerItem.transactions.length" v-model="innerItem.text" class="form-control"></textarea>

                                                                <textarea outlined dense v-else v-model="innerItem.transactions[0].pivot.text" class="form-control"></textarea>
                                            ...
                                            <td v-if="!item.isText" class="text-center" style="width:35%">
                                                <div v-if="!PrintingMood">
                                                    ...
                                                    <v-autocomplete item-text="label" item-value="code" dense outlined v-if="item.transactions.length" @input="UpdateExistingTransactionArchiveRecord(item,item.transactions[0].pivot.status)" :items="AvailableRevisingGuides" v-model="item.transactions[0].pivot.reference" style="width: 100%"></v-autocomplete>
                                                    <v-autocomplete item-text="label" item-value="code" dense outlined v-else :items="AvailableRevisingGuides" v-model="item.default_reference" style="width: 100%" @input="AddNewTransactionArchiveRecord(item,false)"></v-autocomplete>
                        ...
                        <div v-else-if="SelectedItem.code == 'ن6'" class="col-md-12">
                            ...
                            <div class="col-md-12" v-for="(item,index) in SelectedItem.children">
                                <div v-if="item.code == 'ن6-1'">
                                    ...
                                    <textarea outlined dense v-if="!PrintingMood" disabled class="form-control" :text="Transaction.institution.business_activity"></textarea>
                                    <textarea outlined dense v-else disabled class="form-control">{{Transaction.institution.business_activity}}</textarea>
                                ...
                                <div v-if="item.code == 'ن6-2'">
                                    ...
                                    <div v-else>
                                        <h5 class="pt-20">لم يتم تحديد مندوب للمنشأة</h5>
                                    </div>
                                </div>
                                ...
                                <div v-if="item.code == 'ن6-4'">
                                    ...
                                    <div v-if="!PrintingMood">
                                        <textarea outlined dense v-if="!item.transactions.length" v-model="item.text" class="form-control"></textarea>
                                        <textarea outlined dense v-else v-model="item.transactions[0].pivot.text" class="form-control"></textarea>
                                        <v-btn @click="[item.transactions.length ? UpdateExistingTransactionArchiveRecord(item,1) : AddNewTransactionArchiveRecord(item,1)]" class="ma-4" color="primary" dark>
                                            {{$t('save')}}
                                        </v-btn>
                                    </div>
                                    <div v-else>
                                        <textarea outlined dense v-if="!item.transactions.length" class="form-control">{{item.text}}</textarea>
                                        <textarea outlined dense v-else class="form-control">{{item.transactions[0].pivot.text}}</textarea>
                                    ...
                                    <ul>
                                        <li class="pt-10" v-for="ChildItem in item.children">
                                            ...
                                            <div v-if="ChildItem.isText">
                                                <div v-if="!PrintingMood">
                                                    <textarea outlined dense v-if="!ChildItem.transactions.length" v-model="ChildItem.text" class="form-control"></textarea>
                                                    <textarea outlined dense v-else v-model="ChildItem.transactions[0].pivot.text" class="form-control"></textarea>
                                                    <v-btn @click="[ChildItem.transactions.length ? UpdateExistingTransactionArchiveRecord(ChildItem,1) : AddNewTransactionArchiveRecord(ChildItem,1)]" class="ma-4" color="primary" dark>
                                                        {{$t('save')}}
                                                    </v-btn>
                                                </div>
                                                <div v-else>
                                                    <textarea outlined dense v-if="!ChildItem.transactions.length" class="form-control">{{ChildItem.text}}</textarea>
                                                    <textarea outlined dense v-else class="form-control">{{ChildItem.transactions[0].pivot.text}}</textarea>
...
export default {
    ...
    methods: {
        ...
        GetNestedRevisingGuides() {
            ...
            axios.get(route('RevisingGuid.index.archive', {
                    TransactionsID: this.Transaction.id,
                    WithRelation: true
                }))
                .then(({
                    data
        ...
        GetManagingDirector(endpoint) {
            ...
            axios.get(endpoint)
                .then(({
                    ...
                    console.log(error);
                    this.$toast.error('.', 'خطأ يرجى المحاولة مرة أخرى', {
                        timeout: 3000
                    });
            ...
            axios.get(route('documents.index', [this.Transaction.id, this.SelectedItem.code]))
                .then(({
                    ...
                    if (!data.RequiredDocuments[0].transactions.length) {
                        this.$toast.warning('.', 'لا يوجد ملفات', {
                            timeout: 3000
                        });
                    } else {
