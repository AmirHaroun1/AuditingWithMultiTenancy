<template>
    <div>
        ...
        <div class=" mt-20 " style="direction:rtl">
            ...
            <table v-for="(MonetaryItem,MonetaryItemIndex) in Account.MonetaryItems" class="mt-20 table table-bordered" >
                <thead v-if="MonetaryItem.branched_statements && MonetaryItem.branched_statements.length" >
                <tr>
                    <th colspan="7">
                        <h5>
                            {{Account.ClearanceNumber + '.0'}}{{MonetaryItemIndex+1}}
                            {{MonetaryItem.name}}
                        </h5>
                    </th>
                </tr>
                <tr>
                    ...
                    <th rowspan="2" colspan="1" v-if="Account.is_related_party">الفرع</th>
                    <th rowspan="2" colspan="1" v-if="Account.is_related_party">نوع و<br>طبيعة العلاقة</th>
                ...
                <tbody >
                <tr v-for="(item,index) in MonetaryItem.branched_statements">

                    <td>
                        <h5>
                            {{Account.ClearanceNumber + '.0'}}{{MonetaryItemIndex+1}}{{'0'+(index+1)}}
                        </h5>
                    </td>
                    <td>
                        <h5>{{item.name}}</h5>
                    ...
                    <td>
                        ...
                        <input v-model="item.edited_current_year_debtor" class="form-control" type="text" v-if="item.EditValue" v-on:keyup.enter="EditBranchedAccountStatement(item, 'EditValue',MonetaryItem)" >
                    </td>
                    <td>
                        <h5 class="cursor-pointer" @click="$set(item,'EditValue', true)" v-text="formatValue(item.edited_current_year_creditor)"></h5>
                        <input v-model="item.edited_current_year_creditor" class="form-control" type="text" v-if="item.EditValue" v-on:keyup.enter="EditBranchedAccountStatement(item, 'EditValue',MonetaryItem)" >
                    </td>
                    ...
                    <td >
                        <h5 v-text="formatValue(( (item.edited_current_year_creditor - item.edited_current_year_debtor) + (item.current_year_creditor-item.current_year_debtor ) )* 0.1)"></h5>
                    </td>
                </tr>
                <tr>
                    ...
                    <td><h5 v-text="formatValue(MonetaryItem.edited_current_year_creditor)"></h5></td>
                    <td >
                        <h5 class="text-center" v-text="formatValue(MonetaryItem.current_year)"></h5>
                    </td>
        ...
        <div v-if="Account.ShowAccounting_term" class="row mt-20 " style="direction:rtl;margin-top:20px">
...
<script>
    export default {
        name: "ClearanceOfClearancesTable",
        props:{
            'Account':'',
            'Transaction':'',
        },
        ...
        methods : {
            ...
            EditBranchedAccountStatement(BranchedItem, status,parentAccount) {
                this.LoadingSpinner = true;
                this.RecalculateParentEditValues(parentAccount);
                let formData = new FormData();
                formData.append('_method',"PATCH");

                formData.append('parent_id',parentAccount.id);
                formData.append('Parent_FinalCredit',parentAccount.current_year);
                formData.append('Parent_edited_current_year_creditor',parentAccount.edited_current_year_creditor);
                formData.append('Parent_edited_current_year_debtor',parentAccount.edited_current_year_debtor);

                formData.append('Branched_FinalCredit', (BranchedItem.edited_current_year_creditor - BranchedItem.edited_current_year_debtor)+(BranchedItem.current_year_creditor - BranchedItem.current_year_debtor) );
                formData.append('Branched_edited_current_year_creditor',BranchedItem.edited_current_year_creditor == null ? 0:BranchedItem.edited_current_year_creditor);
                formData.append('Branched_edited_current_year_debtor',BranchedItem.edited_current_year_debtor == null ? 0:BranchedItem.edited_current_year_debtor);
                axios.post(route('accounts.statements.branches.update',BranchedItem.id),formData)
                    .then(({data})=>{
                        console.log(data);
                        this.$toast.success('.','تم التعديل بنجاح',{timout:300});
                        BranchedItem.current_year = parseInt(BranchedItem.edited_current_year_creditor - BranchedItem.edited_current_year_debtor)+ parseInt(BranchedItem.current_year_creditor - BranchedItem.current_year_debtor);
                        this.LoadingSpinner = false;
                ...
                BranchedItem.EditValue = false;

            },
            RecalculateParentEditValues(parentAccount){
                parentAccount.edited_current_year_creditor = 0;
                parentAccount.edited_current_year_debtor = 0;
                parentAccount.branched_statements.forEach(Branch => {
                    parentAccount.edited_current_year_creditor += parseInt(Branch.edited_current_year_creditor);
                    parentAccount.edited_current_year_debtor += parseInt(Branch.edited_current_year_debtor);
                })
                parentAccount.current_year = parseInt(parentAccount.edited_current_year_creditor - parentAccount.edited_current_year_debtor)+ parseInt(parentAccount.current_year_creditor - parentAccount.current_year_debtor);
            },
